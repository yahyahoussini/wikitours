...
-- ---- 023 — evergreen month landers + departure lifecycle from data ---------
...
create policy anon_read on public.month_pages for select to anon using (true);
...
-- the lifecycle (live → archived → retired) is code; RLS no longer hides the past
drop policy if exists anon_read on public.offers;
create policy anon_read on public.offers for select to anon using (is_published);
drop policy if exists anon_read on public.offer_tiers;
create policy anon_read on public.offer_tiers
  for select to anon
  using (is_published and exists (select 1 from public.offers o where o.id = offer_tiers.offer_id and o.is_published));

-- ---- 024 — authors (E-E-A-T): profile fields on team_members, author/reviewer on articles
alter table public.team_members
  ...
  add column if not exists bio_fr text, add column if not exists bio_ar text, add column if not exists bio_en text,
  add column if not exists credentials_fr text, add column if not exists credentials_ar text, add column if not exists credentials_en text,
