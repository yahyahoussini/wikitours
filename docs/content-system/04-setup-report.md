...
| `clusterOfPath()` returned the first cluster whose pillar **or** pages matched | `/omra-ramadan` is the pillar of cluster D but sits in cluster A's `pages`, so a Ramadan article was asked for cluster A siblings — all commercial pages G6 forbids linking. **The rule was unsatisfiable.** Live impact was on ARTICLE pages, not the hubs (which render `RelatedArticles`, not `ClusterLinks`): `/blog/prix-omra-maroc-par-gamme-et-mois` showed cluster A's pillar and siblings instead of cluster C's | A pillar match wins; `tests/clusters.test.mjs` pins every pillar. Verified on production: that article now renders `data-cluster="C"` |
