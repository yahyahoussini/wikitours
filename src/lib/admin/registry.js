...
export const ADMIN_ENTITIES = {
  ...
  articles: {
    ...
    fields: [
      ...
      { name: 'author_id', type: 'rel', label: 'Auteur (membre de l\'équipe — byline + schéma Person)', relTable: 'team_members', relLabel: 'name' },
      { name: 'reviewer_id', type: 'rel', label: 'Relu par (membre de l\'équipe — articles rituels / santé)', relTable: 'team_members', relLabel: 'name' },
      { name: 'author_name', type: 'text', label: 'Auteur (texte libre, affiché si aucun membre choisi)' },
      { name: 'reviewed_by', type: 'text', label: 'Relu par (texte libre)' },
      { name: 'published_at', type: 'datetime', label: 'Date de publication' },
      { name: 'supports_path', type: 'text', label: 'Page soutenue (ex. /omra-pas-cher) — l’article y est listé « Pour aller plus loin »' },
    ],
  },

  'plan-articles': {
    table: 'article_plan',
    title: 'Plan éditorial (blog IA)',
    publishField: 'is_active',
    orderBy: 'sort_order',
    listColumns: ['query_family', 'status', 'owner_path', 'season_month', 'sort_order'],
    searchKeys: ['query_family', 'angle', 'owner_path', 'notes'],
    hasSeo: false,
    hasGallery: false,
    labelField: 'query_family',
    duplicateDisabled: true,
    fields: [
      { name: 'query_family', type: 'text', label: 'Famille de requêtes que l’article possédera (ex. « omra parents âgés »)', required: true },
      { name: 'angle', type: 'textarea', label: 'Angle / consigne au rédacteur (facultatif)' },
      {
        name: 'category',
        type: 'select',
        label: 'Catégorie',
      ...
      { name: 'season_month', type: 'number', label: 'Mois visé 1–12 (rédigé ≤ 3 mois avant ; vide = hors saison)' },
      {
        ...
        options: [
          ...
          { value: 'drafted', label: 'Brouillon produit' },
          { value: 'skipped', label: 'Ignoré' },
        ],
      },
      { name: 'article_id', type: 'rel', label: 'Article produit', relTable: 'articles', relLabel: 'title_fr' },
      { name: 'notes', type: 'textarea', label: 'Notes (le rédacteur y consigne ses erreurs)' },
      { name: 'sort_order', type: 'number', label: 'Ordre' },
    ],
  },
  ...
  voyages: {
    ...
    publishField: 'is_published',
    orderBy: 'sort_order',
    listColumns: ['title_fr', 'destination', 'date_start', 'starting_price'],
    searchKeys: ['title_fr', 'title_ar', 'title_en', 'slug', 'destination'],
    hasSeo: true,
    hasGallery: true,
    labelField: 'title_fr',
    fields: [
      { name: 'slug', type: 'text', label: 'Slug', required: true },
      { name: 'title', type: 'text3', label: 'Titre' },
      { name: 'destination', type: 'text', label: 'Destination (ex. Istanbul, Turquie)' },
      { name: 'date_start', type: 'date', label: 'Date départ' },
      { name: 'date_end', type: 'date', label: 'Date retour' },
      { name: 'duration_days', type: 'number', label: 'Durée (jours)' },
      { name: 'duration_nights', type: 'number', label: 'Nuits' },
      { name: 'starting_price', type: 'number', label: 'Prix à partir de (MAD/pers)' },
      { name: 'airline', type: 'text', label: 'Compagnie aérienne' },
      { name: 'summary', type: 'textarea3', label: 'Résumé' },
      { name: 'description', type: 'md3', label: 'Description (markdown)' },
      { name: 'inclusions', type: 'textarea3', label: 'Inclusions (une ligne par élément)' },
      { name: 'exclusions', type: 'textarea3', label: 'Non inclus (une ligne par élément)' },
      { name: 'conditions', type: 'textarea3', label: 'Conditions' },
      { name: 'is_featured', type: 'bool', label: 'En vedette' },
      { name: 'sort_order', type: 'number', label: 'Ordre' },
    ],
  },
  ...
  equipe: {
    ...
    hasGallery: true,
    labelField: 'name',
    // Authors / E-E-A-T (migration 024). A profile renders on /equipe and as a
    // byline only when it is published, NOT « fiche provisoire », has a slug
    // and a bio — see docs/authors-intake.md for what each person must supply.
    fields: [
      { name: 'name', type: 'text', label: 'Nom (tel qu\'affiché sur /fr)', required: true },
      { name: 'name_ar', type: 'text', label: 'Nom en arabe (affiché sur /ar)' },
      { name: 'name_en', type: 'text', label: 'Nom en anglais (si différent)' },
      { name: 'slug', type: 'text', label: 'Slug (identifiant stable, ex. aya-tahiri — ne plus changer une fois publié)' },
      { name: 'role', type: 'text3', label: 'Rôle' },
      { name: 'years_experience', type: 'number', label: 'Années d\'expérience Omra / Hajj' },
      { name: 'languages', type: 'text', label: 'Langues parlées (ex. français, arabe, darija, anglais)' },
      { name: 'bio', type: 'textarea3', label: 'Bio (60–120 mots, factuelle — requise pour afficher le profil)' },
      { name: 'credentials', type: 'textarea3', label: 'Formations / références (uniquement si réelles et vérifiables)' },
      { name: 'sameas_url', type: 'text', label: 'Profil public (LinkedIn…) — schéma Person sameAs' },
      { name: 'is_placeholder', type: 'bool', label: 'Fiche provisoire — jamais affichée ni émise en schéma' },
      { name: 'sort_order', type: 'number', label: 'Ordre' },
    ],
  },
